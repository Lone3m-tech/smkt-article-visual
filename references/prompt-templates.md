...
All non-highlighted title characters are deep ink. The sole selection field keeps its thin brand-green outline and may end with up to two small terminal dots; it is the title-level brand-green emphasis, while any separate carrier accent is governed only by the page colour direction. This treatment may change the rhythm of the one exact title, but must never add, omit, reorder, paraphrase, or duplicate title characters. It is not a body annotation system, interface, diagram, or source claim.
```
