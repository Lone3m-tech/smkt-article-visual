...
Require Python 3.10+ and the pinned Pillow dependency, then finalize every generated path. The finalizer automatically detects the exact regular PNG named by `dimensions.logo.asset` in `assets/logo/`. It overlays that transparent Logo when found; when it is absent or a symlink, it skips Logo application. It preserves the generated canvas, cover title, all body title regions, and the content stage unchanged: it does not crop, clear, replace, or reflow any title, subtitle, or explanatory artwork.
