...
description: >
  ...
  generate tactile editorial diagrams with image_gen as either an article package placed at exact anchors or standalone presentation frames. Use for 给文章配封面和正文解释图、给演讲稿做讲述图、给提案或报告做解释图、给文章做配图框架、补正文配图、重做文章配图 or 图文并茂.
...
`source_path` is the only required content input. Accept `article_path` as a backwards-compatible alias for `source_path`. `article_package` is the default and requires an article-like Markdown source; use `presentation_frames` for a talk, report, proposal, or workshop outline and never insert Markdown references into that source. Before planning or generating, read `assets/simplemkt-editorial-style.md`: it supplies the shared SimpleMkt editorial art direction and the separate cover/body composition rules. The cover, body figures, or presentation frames must look like one series through their white canvas, palette, typography, linework, material cues, and wordmark, but they must not use the same layout. Pure white constrains the canvas only; it does not mean outline-only line art. Treat the shared art direction as the default visual language.
...
- In `presentation_frames`, do not create a cover by default. Save each accepted frame as `<source_root>/assets/image/<frame-id>.png`, ordered by its approved narrative sequence. Use the source only to extract the judgment and its evidence; do not insert image references.
...
```json
{
  ...
  "images": [{
    ...
    "placement": {"kind": "after_h1 | after_paragraph | narrative_order", "anchor": "..."},
    "accepted_attempt": 2,
    "attempts": [{
      "id": 1,
...
Do not add a figure merely because a heading or paragraph exists. A cover is required only for `article_package`; every body figure or presentation frame must clarify a mechanism, conditional choice, comparison, overlap, argument, sequence, feedback loop, boundary, hierarchy, temporal progression, continuum, or authorized source image.
...
- In `presentation_frames`, compile one independent prompt per accepted frame. Use the frame's `key_judgment` as its centered core judgment and its approved `sequence` as the output order. Do not generate a cover or turn the frame into a generic slide template.
...
- Create one image record per planned figure or frame with `id`, `role`, `output_path`, and exact placement or narrative order.
...
- In `presentation_frames`, preserve source prose unchanged and verify each frame file exists once in approved narrative order.
...
- Re-read the saved source. In `article_package`, verify every planned path resolves once and appears once at its planned location. In `presentation_frames`, verify no image Markdown was added and every approved frame path resolves once before reporting completion.
...
- In `presentation_frames`, verify there is one resolving frame file for every approved sequence item, the source prose is unchanged, and no image Markdown was inserted.
...
Do not report generation or delivery complete merely because image files exist. Complete only when the framework was accepted or generation was explicitly requested; in `article_package`, the saved article contains one resolving cover and one resolving image for every planned figure at exact anchors; in `presentation_frames`, every approved frame resolves once in its approved order and the source remains unchanged; all assets and the selected delivery mode pass QA; and no planned image is missing or unrelated. Image models are probabilistic, so this guarantees the same visual contract—not byte-identical pixels across model versions.
